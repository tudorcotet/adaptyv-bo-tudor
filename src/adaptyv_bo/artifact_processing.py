...
def process_csvs(experiment_name, output_dir = './output_benchmark_csv'):
...
def run_artifact_processing_pipeline(experiment_name, output_dir = './output_benchmark_csv'):
    ...
    combined_df = pd.read_csv(os.path.join(output_dir, 'combined_metrics.csv'))
    # Filter out runs with less than 5 seeds
    # Count unique seeds per combination
    seed_counts = aggregated_df.groupby('combination')['seed'].nunique()
    
    # Filter combinations with at least 5 seeds
    valid_combinations = seed_counts[seed_counts >= 5].index
    
    # Filter out combinations where all seeds have the same fitness per round
    def has_varying_fitness(group):
        return group.groupby('round')['fitness'].nunique().gt(1).any()
    varying_fitness_combinations = aggregated_df.groupby('combination').filter(has_varying_fitness)['combination'].unique()
    
    # Combine both filters
    final_valid_combinations = set(valid_combinations) & set(varying_fitness_combinations)
    
    # Apply the combined filter to the dataframe
    aggregated_df = aggregated_df[aggregated_df['combination'].isin(final_valid_combinations)]
    if aggregated_df.empty:
        print("Warning: No combinations with 5 or more seeds found. Proceeding with all data.")
        aggregated_df = pd.concat(dfs).reset_index()
    else:
        print(f"Filtered out {len(seed_counts) - len(valid_combinations)} combinations with less than 5 seeds.")
    ...
    sns.lineplot(aggregated_df[aggregated_df['combination'].isin(top_5_models)], x = 'round', y = 'fitness', errorbar = 'sd', hue = 'combination')
    plt.show()
...
def aggregate_by_model_combination(df):
    ...
    df.loc[df['Kernel'].isna(), 'Kernel'] = 'none'
    df.loc[df['Surrogate'].isna(), 'Surrogate'] = 'none'
    df.loc[df['Acquisition'].isna(), 'Acquisition'] = 'none'
...
def analyze_model_efficiency(aggregated_df):
    ...
    return efficiency_metrics.sort_values('final_fitness', ascending=False)
...
if __name__ == "__main__":
    ...
    run_artifact_processing_pipeline('https://mlflow.internal.adaptyvbio.com/2024/02_adaptyv_bo_tudor/002_benchmark_test')
